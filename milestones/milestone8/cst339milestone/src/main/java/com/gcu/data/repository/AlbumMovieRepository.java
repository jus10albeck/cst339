...
public interface AlbumMovieRepository extends CrudRepository<AlbumMovieEntity, Long>
{
    ...
    """)
    boolean existsMovieNameInAlbum(@Param("usernameHash") byte[] usernameHash,
    ...
    @Query("""
        ...
        JOIN `user` u ON u.id = a.user_id
        JOIN login l ON l.id = u.login_id
        WHERE l.username_hash = :usernameHash AND a.id = :albumId AND am.movie_id = :movieId
    """)
    int deleteFromUserAlbum(@Param("usernameHash") byte[] usernameHash,
                            @Param("albumId") Long albumId,
                            @Param("movieId") Long movieId);
    ...
    @Query("""
        ...
        JOIN `user` u ON u.id = a.user_id
        JOIN login l ON l.id = u.login_id
        WHERE l.username_hash = :usernameHash AND a.id = :albumId
    """)
    int deleteAllFromUserAlbum(@Param("usernameHash") byte[] usernameHash,
                               @Param("albumId") Long albumId);
}